...
# ===========================================================================
# 5. dist_data_loader sampler types
# ===========================================================================


class TestDistDataLoaderSamplers:
    """Verify dist_data_loader maps shuffle argument to explicit samplers."""

    def test_shuffle_true_uses_subset_random_sampler(self, monkeypatch, tmp_path):
        """shuffle=True selects SubsetRandomSampler; legacy shuffle key not forwarded."""
    ...
    def test_shuffle_false_has_no_random_sampler(self, monkeypatch, tmp_path):
        """shuffle=False passes sampler=None so PyTorch uses its default SequentialSampler."""
        from hyrax import pytorch_ignite
        from hyrax.pytorch_ignite import dist_data_loader
        ...
        dist_data_loader(dp, config, shuffle=False)

        assert captured["sampler"] is None
        assert "shuffle" not in captured["kwargs"]

    def test_shuffle_true_builds_generator_on_idist_device(self, monkeypatch, tmp_path):
        """Random sampler generator is created on the Ignite-selected device."""
        import torch
        ...
        monkeypatch.setattr(pytorch_ignite.idist, "auto_dataloader", fake_auto_dataloader)
        monkeypatch.setattr(pytorch_ignite.idist, "device", lambda: torch.device("cpu"))
        ...
        assert captured["generator_device"] == torch.device("cpu")
