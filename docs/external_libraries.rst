...
it's ``x`` input. See the example below, which stacks the g, r, and i fluxes into a single numpy array:
...
Dataset classes are written as subclasses of ``hyrax.datasets.HyraxDataset``. Datasets must choose to be
...
numpy array or a dictionary of named data values that could be converted into a numpy array by the
...
data into an array for ML applications, we recommend returning a numpy array.  If there are multiple ways
...
model a numpy array and an optional label. If these keys do not exist, Hyrax will prompt that a ``prepare_inputs``
