...
def setup_dataset(config: ConfigDict, tensorboardx_logger: Optional[SummaryWriter] = None) -> Dataset:
    ...
    if is_iterable_dataset_requested(data_request):
        ...
        dataset_class = data_definition["dataset_class"] if "dataset_class" in data_definition else None
        if dataset_class is None:
            raise RuntimeError("dataset_class must be specified in the data_request for iterable datasets.")
        else:
            dataset_cls = DATA_SET_REGISTRY[data_definition["dataset_class"]]

        data_location = data_definition["data_location"] if "data_location" in data_definition else None
