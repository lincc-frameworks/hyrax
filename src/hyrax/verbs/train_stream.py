...
class TrainStream(Verb):
    ...
    def run(self) -> "TrainStreamSession":
        ...
        from hyrax.tensorboardx_logger import init_tensorboard_logger

        config = self.config

        # Build the model either from a configured streaming dataset
        provider = None
        data_loader = None
