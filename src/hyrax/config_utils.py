...
def log_mlflow_params(config: dict, results_dir: Path, verb: str):
    """Log parameters to MLflow from the config file.
    ...
    verb : str
        The verb name (e.g., "train", "test") for logging specific parameters
    """
    import mlflow
    ...
    # Log verb-specific parameters
    if verb == "train":
        mlflow.log_param("epochs", config["train"]["epochs"])

        # Log the optimizer params
