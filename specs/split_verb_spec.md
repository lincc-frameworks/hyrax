...
1. Split files store **integer index lists** per split + a **fingerprint** (dataset length,
   class, location, seed, fractions, stratify field, probed object IDs).
2. A premade split is referenced by **extending `data_request` per group** with a single new
   ...
   group points only at the file it consumes (no `split_name` needed).
...
- `compute_inverse_frequency_weights(labels, train_indices) -> list[float]` — per-sample weights
  **positionally aligned to `train_indices`** (count classes over train only; `weight ∝ 1/count`).
...
- `save_split(output_dir, split_indices, train_weights, meta) -> dict[str, Path]` — writes
  **one `<split>.npz` per split** (e.g. `train.npz`, `validate.npz`, `test.npz`) into
  `output_dir`, each self-describing; returns `{split_name: path}`.
- `load_split(path) -> (indices, weights, meta)` — loads a single split file.
...
and — for `train.npz` only — `weights` (float32). Per-split files match the user's "files
...
4. `providers = setup_dataset(config, splits=tuple(fractions))`; the split groups share one
   underlying dataset, so assert they agree on `primary_data_location`/`len` and take one as the
   source `provider`. `n = len(provider)` (full dataset length — `__len__` returns the primary
   dataset size regardless of any `split_indices` the partition loop set, verified in
   `data_provider.py:383`); `seed = config["data_set"]["seed"] or None`.
5. If `stratify_field`: resolve `provider.dataset_getters[provider.primary_dataset][stratify_field]`
   ...
   fall back to random). Else `labels=None`.
6. `split_indices = compute_split_indices(n, fractions, seed=seed, labels=labels)`.
7. `train_weights = compute_inverse_frequency_weights(labels, split_indices["train"])` when
   `weights and labels is not None and "train" in split_indices`, else `None` (weights apply to
...
8. Build `meta` (fingerprint via `provider.get_object_id` as `id_getter`, plus a `split_set_id`)
   → `save_split(results_dir, split_indices, train_weights, meta)` → return `results_dir` (the
...
`@field_validator` that resolves the path like `resolve_data_location`, local-only). Add
`@model_validator`s: `split_file` XOR `split_fraction` (mutually exclusive); `split_file`
...
- **`setup_dataset`** (`:53`): after providers are built (~`:95`) and before the `split_fraction`
  ...
  `(indices, weights, meta) = load_split(Path(split_file))` then
  `verify_split_compatible(meta, provider, config, group_name=...)`, and set
  `provider.split_indices = indices` and `provider.sample_weights = weights` (only `train.npz`
  carries weights; others load `None`). Premade-split groups have no `split_fraction`, so the
  existing partition loop (`:104`) already skips them.
- **`DataProvider.__init__`** (`data_provider.py:318`): add `self.sample_weights = None` next to
  `self.split_indices = None`.
- **`dist_data_loader`** (`:222` boolean-split branch): when `dataset.split_indices` is set and
  `dataset.sample_weights is not None and shuffle`, wrap in `torch.utils.data.Subset(dataset,
  ...
  aligned — guaranteed by `compute_inverse_frequency_weights`. Validate/test stay deterministic
  (weights only on train + branch requires `shuffle=True`, which Train passes only for train).
...
Unit: random proportions; determinism (seed); stratified proportions per split; inverse-freq
weights (length/alignment/ratio); fingerprint sensitivity; save/load roundtrip (per-split files;
only `train.npz` has weights); `verify_split_compatible` mismatch raises with named field;
missing/None label raises. Integration: `Hyrax().split()` writes
...
`WeightedRandomSampler` and validate/test are `SubsetSequentialSampler`. Add data_request
...
proportions), set `config["split"]["stratify_field"]="label"`, run `h.split()` (returns a dir
holding `train.npz`/`validate.npz`/`test.npz`); then switch each group from `split_fraction` to a
`split_file` pointing at the matching file; run `h.train()` for 1 epoch and confirm it consumes
the split and uses `WeightedRandomSampler` for train. 
...
- **`WeightedRandomSampler` position-vs-index trap**: solved by `Subset` + positional weight
  alignment (must pass `collate_fn=dataset.collate`).
